...
$('.logout').click( function(e) {
    ...
    localStorage.removeItem('setmyprofile');
...
//--- Bootstrap 3 Modal external URL --
    ...
    $(document).on('click', '[data-load-remote]', function(e) {    
        ...
        var $this = $(this);
        modal_remote_link = $this.data('load-remote');

        //alert("link [data-load-remote] : "+modal_remote_link);
        //console.log("link  : "+modal_remote_link);
   
            if(modal_remote_link) {
                $($this.data('remote-target')).load(modal_remote_link);
            }
...
$('a.myprofile').on('click', function(e) {
    ...
    //alert("Set Myprofile");
    console.log("Set MyProfile");
    
    localStorage.setItem('setmyprofile','profile');      
...
$('a.setpwd').on('click', function(e) {
    ...
    localStorage.setItem('setmyprofile','password');
...
//-- shown model already --
 $("#myModal").off().on("shown.bs.modal", function() {
    ...
    console.log("myModal shown : "+modal_remote_link);  
    
        if (modal_remote_link === 'myprofile.html') {
            ...
            if (localStorage.getItem('setmyprofile')==='profile') {
                $("form.user-profile-form").show();
                $("form.user-setpwd-form").hide(); 
                $('h3#lineModalLabel-profile-form.modal-title-profile').html('<i class="fa fa-user"></i> Edit Profile');
            }

            if (localStorage.getItem('setmyprofile')==='password') {
                $("form.user-profile-form").hide();
                $("form.user-setpwd-form").show(); 
                $('h3#lineModalLabel-profile-form.modal-title-profile').html('<i class="fa fa-user"></i> Account Settings');
            } 

         }
     
        if (modal_remote_link === 'repair_com.html') {

           //--- show cust info in repair.html ---
           $("p.cust-info").html("<h4> <b> customer : </b> "+role.name+"    "+role.lastname+"</h4>");
           $("input.cust-info#iduser").val(role.iduser);

           //--- show device type COM Unit ---
           shDevice_type(1);
           
        }
          
     
        if (modal_remote_link === 'addjob_com.html') {
           
           shDevice_type(1);
           shDept_select();
           shServicetype_select();
           shServicemethod_select();
           shBuilding_select();
           shOS_select();
           shStatus_select();
...
//-- hidden model 'have to' clear div modal-body also --
...
$(document).on('click', "a.menu-bar", function() {
    ...
    localStorage.setItem("current-src", url);   
    localStorage.setItem("setmyprofile", "profile");  // set profile or set pwd;
...
function change_unit(unit,filename){
    
    console.log("Select unit :"+unit);
   
    $('.navbar-brand').text(":: "+unit+" ::");       
     
    //$('a.newjob-menubar').attr("data-load-remote","addjob_"+filename+".html");
    
    //alert("change unit :"+$('a.newjob-menubar').attr("data-load-remote"));
    
    $('li.addnewjob').html('');
    
    //alert($('li.addnewjob').html());
    
    newjob = "addjob_"+filename+".html";
    
    var new_addjobmenu = "<a href='#myModal' role='button' class='newjob-menubar' data-toggle='modal' data-load-remote='"+newjob+"' data-remote-target='#myModal .modal-body'> New Job </a>";
    
    $('li.addnewjob').html(new_addjobmenu);
...
function loadmenubar(){
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                document.getElementById("repair-menu").innerHTML = html; 
...
function shDevice_type(unit){
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                var html="<option value='0'>-- เลือกชนิดอุปกรณ์ --</option>";
...
function shDept_select(){
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                var html="<option value='0'>-- เลือกสังกัดกอง --</option>";
                
                for(var key in shData.data) {
                    
                    html += "<option value=" + shData.data[key].id + ">" +shData.data[key].shortname + "</option>";
                    
                }
                
                document.getElementById("iddept").innerHTML = html; 
...
function shBuilding_select(){
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                var html="<option value='0'>-- เลือกอาคาร --</option>";
                ...
                document.getElementById("idbuilding").innerHTML = html; 
...
function shOS_select(){
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                var html="<option value='0'>-- เลือกระบบปฎิบัติการ --</option>";
                ...
                document.getElementById("idos").innerHTML = html; 
...
function shServicetype_select(){
    
    $.ajax({
            type:'GET',
            url: webapp+'servicetype' ,
            contentType: "application/json",
            success: function (data) {
                // data = JSON.parse(data);
                //data = JSON.stringify(data);
                ...
                var html="<option value='0'>-- เลือกชนิดงานบริการ --</option>";
                ...
                document.getElementById("idservice_type").innerHTML = html; 
...
function shServicemethod_select(){
    
    $.ajax({
            type:'GET',
            url: webapp+'servicemethod' ,
            contentType: "application/json",
            success: function (data) {
                // data = JSON.parse(data);
                //data = JSON.stringify(data);
                ...
                var html="<option value='0'>-- เลือกวิธีงานบริการ --</option>";
                ...
                document.getElementById("idservice_method").innerHTML = html; 
...
function shStatus_select(){
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                var html="<option value='0'>-- เลือกสถานะงาน --</option>";
                ...
                document.getElementById("idstatus").innerHTML = html; 
...
function check_device_type() {
   ...
   select_label = $('select#iddevice_type option:selected').text();
   
   //alert("Select Device : "+value);
   //alert("Select Device : "+select_label);
      
    if ((select_label === 'PC')||(select_label === 'Notebook')){
